...
- **No network restrictions.** SSH agent forwarding is available if the host has `SSH_AUTH_SOCK` set. No raw SSH keys or GPG keys are mounted.
...
- **Git:** Full git operations within the project directory. SSH agent forwarding enables git-over-SSH (push, pull, clone from private repos)
